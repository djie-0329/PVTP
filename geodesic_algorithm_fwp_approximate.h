#ifndef GEODESIC_ALGORITHM_FWP_APPROXIMATE_H
#define GEODESIC_ALGORITHM_FWP_APPROXIMATE_H
...
namespace geodesic{
	...
	class GeodesicAlgorithmFWPApproximate : public GeodesicAlgorithmBase
	{
	public:
		GeodesicAlgorithmFWPApproximate(geodesic::Mesh* mesh, double lambda) :
			GeodesicAlgorithmBase(mesh),
			lambda(lambda),
			m_vertex_FWP_queue(mesh->vertices().size(), 1)
		{};
		~GeodesicAlgorithmFWPApproximate(){};
	...
	//----------------------simple functions-------------------
	inline void GeodesicAlgorithmFWPApproximate::initialize_propagation_data()
	{
		clear();
	...
	inline void GeodesicAlgorithmFWPApproximate::erase_from_queue(vertex_pointer v)
	{
		/*
	...
	inline void GeodesicAlgorithmFWPApproximate::erase_from_FWP_queue(vertex_pointer v)
	{
		if (v->ptrInQueue)
		{
	...
	inline void GeodesicAlgorithmFWPApproximate::create_pseudo_source_windows(vertex_pointer &pseudo_source, bool inside_traversed_area)
	{
		...
		for (unsigned i = 0; i < pseudo_source->adjacent_edges().size(); ++i)
		{
			...
			if (distance < vert_it->geodesic_distance())
			{
				...
				if (vert_it->state() == Vertex::INSIDE && vert_it->saddle_or_boundary())
				{
					/*
					depth_create_pseudo++;
					if (depth_create_pseudo > mx_depth)
					{
						//printf("rings: %d, depth: %d\n", num_rings, depth_create_pseudo);
						mx_depth = depth_create_pseudo;
					}
					*/
	...
	//----------------- propagate a windows list (Rule 1) ---------------------
	inline void GeodesicAlgorithmFWPApproximate::find_separating_point(list_pointer &list)
	{
		...
		if (top_t.geodesic_distance() < Tri.top_vertex->geodesic_distance())
		{
			if (Tri.top_vertex->state() == Vertex::FRONT)
				erase_from_FWP_queue(Tri.top_vertex);
	...
	inline void GeodesicAlgorithmFWPApproximate::propagate_windows_to_two_edges(list_pointer &list)
	{
	...
	//----------------- pairwise windows checking (Rule 2) ----------------------
	inline void GeodesicAlgorithmFWPApproximate::check_with_vertices(list_pointer &list)
	{
	...
	inline windows_state GeodesicAlgorithmFWPApproximate::check_between_two_windows(interval_pointer &w1, interval_pointer &w2)
	{
	...
	inline void GeodesicAlgorithmFWPApproximate::pairwise_windows_checking(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//----------------- approximate operation (Du Jie)----------------------------
	inline void GeodesicAlgorithmFWPApproximate::propagate_local_exact_pseudo_source_at_wavefront()
	{
		...
		while (!m_vertex_FWP_queue.empty())
		{
			...
			m_vertex_FWP_queue.remove(vert->ptrInQueue);
		...
		//printf("ring_id: %d, num of exact vertices: %d\n", num_rings, m_local_exact_vertex_queue.size());
		...
		while (!m_local_exact_vertex_queue.empty())
		{
			...
			//push window lists on the wavefront incident to v into m_list_queue
			for (unsigned i = 0; i < vert->adjacent_edges().size(); i++)
			{
				edge_pointer edge_it = vert->adjacent_edges()[i];
			...
			while (!m_list_queue.empty())
			{
				...
				//if models have boundary, this will produce memory leak
				list->clear();
			}
		}
	}
	...
	//------------------------- main operation ----------------------------
	inline void GeodesicAlgorithmFWPApproximate::propagate_one_windows_list(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//----------------------main entry---------------------
	inline void GeodesicAlgorithmFWPApproximate::propagate(unsigned source)
	{
		//initialization
		m_source = source;
	...
	//---------------------- print statistics --------------------------
	inline void GeodesicAlgorithmFWPApproximate::print_statistics()
	{
		...
		FILE *file = fopen("FWP_Appr_VTP.csv", "a");
		fprintf(file, "%.6lf, %d, %d, %d\n", m_time_consumed, num_rings, num_propagated_vertices, m_windows_propagation);
		fclose(file);
	}
