#ifndef GEODESIC_ALGORITHM_FWP_EXACT_H
#define GEODESIC_ALGORITHM_FWP_EXACT_H
...
namespace geodesic {
	...
	class GeodesicAlgorithmFWPExact : public GeodesicAlgorithmBase
	{
	public:

		// basic functions related to class
		GeodesicAlgorithmFWPExact(geodesic::Mesh* mesh) :
			GeodesicAlgorithmBase(mesh),
			//m_memory_allocator(mesh->edges().size(), mesh->edges().size()),
			m_vertex_FWP_queue(mesh->vertices().size(), 1)
			//unProcessedInside(mesh->vertices().size(), false)
		{};
		~GeodesicAlgorithmFWPExact() {};
		void clear() 
		{ 
	...
	private:
		...
		unsigned num_propagated_vertex;
		...
		//----------for debug-----------
		//std::vector<bool> unProcessedInside;

	};

	//----------------- simple functions ---------------------
	inline void GeodesicAlgorithmFWPExact::initialize_propagation_data()
	{
		clear();
	...
	inline void GeodesicAlgorithmFWPExact::erase_from_queue(vertex_pointer v)
	{
	...
	inline void GeodesicAlgorithmFWPExact::erase_from_FWP_queue(vertex_pointer v)
	{
		if (v->ptrInQueue)
		{
	...
	inline void GeodesicAlgorithmFWPExact::create_pseudo_source_windows(vertex_pointer &pseudo_source, bool inside_traversed_area)
	{
		...
		for (unsigned i = 0; i < pseudo_source->adjacent_edges().size(); ++i)
		{
			...
			if (distance < vert_it->geodesic_distance())
			{
				...
				//here will influence accurancy
				//if (vert_it->state() == Vertex::INSIDE && vert_it->saddle_or_boundary())
				//{
					//unProcessedInside[vert_it->id()] = true;
					//printf("propagated vertices num: %d, true inside_id: %d, geodesic: %.20lf\n", num_propagated_vertex, vert_it->id(), vert_it->geodesic_distance());
					//create_pseudo_source_windows(vert_it, true);
				//}
	...
	//----------------- propagate a windows list (Rule 1) ---------------------
	inline void GeodesicAlgorithmFWPExact::find_separating_point(list_pointer &list)
	{
		...
		// update top_vertex and M_VERTEX_QUEUE M_VERTEX_FWP_QUEUE
		if (top_t.geodesic_distance() < Tri.top_vertex->geodesic_distance())
		{
			...
			if (Tri.top_vertex->state() == Vertex::FRONT) erase_from_FWP_queue(Tri.top_vertex);
			...
			{
				Tri.top_vertex->ptrInQueue = m_vertex_FWP_queue.push(Tri.top_vertex, Tri.top_vertex->geodesic_distance());
			}
			...
			if ((Tri.top_vertex->state() == Vertex::INSIDE) && (Tri.top_vertex->saddle_or_boundary()))
			{
				...
				//unProcessedInside[Tri.top_vertex->id()] = false;
				//printf("propagated vertices num: %d, false inside_id: %d, geodesic: %.20lf\n", num_propagated_vertex, Tri.top_vertex->id(), Tri.top_vertex->geodesic_distance());
			}
		}
	...
	inline void GeodesicAlgorithmFWPExact::propagate_windows_to_two_edges(list_pointer &list)
	{
	...
	//----------------- pairwise windows checking (Rule 2) ----------------------
	inline void GeodesicAlgorithmFWPExact::check_with_vertices(list_pointer &list)
	{
	...
	inline windows_state GeodesicAlgorithmFWPExact::check_between_two_windows(interval_pointer &w1, interval_pointer &w2)
	{
	...
	inline void GeodesicAlgorithmFWPExact::pairwise_windows_checking(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//------------------------- main operation ----------------------------
	inline void GeodesicAlgorithmFWPExact::propagate_one_windows_list(list_pointer &list)
	{
		if (list->empty()) return;
	...
	//-------------------------- main entry --------------------------
	inline void GeodesicAlgorithmFWPExact::propagate(unsigned source)
	{
		// initialization
		m_source = source;
		initialize_propagation_data();

		num_propagated_vertex = 0;
		...
		{
			m_fwp_queue_max_size = max((int)m_vertex_FWP_queue.size(), (int)m_fwp_queue_max_size);
			
			m_iterations = m_vertex_FWP_queue.getPhase();
			...
			vertex_pointer vert = m_vertex_FWP_queue.pop();
			vert->ptrInQueue = NULL;
			
			num_propagated_vertex++;
	...
	//---------------------- print statistics --------------------------
	inline void GeodesicAlgorithmFWPExact::print_statistics()
	{
		/*
		for (unsigned i = 0; i < this->mesh()->vertices().size(); i++)
		{
			if (unProcessedInside[i])
			{
				printf("unprocessed inside_id: %d\n", i);
			}
		}*/
		...
		std::cout << "total propagated vertices number " << num_propagated_vertex << std::endl << std::endl;

		//output data to .csv(excel file)
		FILE* file = fopen("FWP_VTP.csv", "a");
		fprintf(file, "%.6lf, %d, %d\n", m_time_consumed, num_propagated_vertex, m_windows_propagation);
		fclose(file);
